...
#' 
#' Concords the Unites State Patent Classification codes (USPC 2012) to and from the International Patent Classification codes (IPC 2012).
#'
#' @param sourcevar An input character vector of USPC or NAICS codes. The function accepts 3-digit codes for USPC (class) and 5 to 11-digit codes for USPC (subclass), and 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 6 to 8-digit codes for IPC (group), and 9 to 12-digit codes for IPC (subgroup).
...
#' }
#' @note Always include leading zeros in USPC codes for class, but not subclass (e.g., use 002 for USPC class, instead of 2, but 002/2.11 for USPC subclass), a space in IPC codes between subclass and group (e.g., use A01B 1 for IPC group, instead of A01B1), and leading zeros in IPC codes between group and subgroup (e.g., use A01B 1/02 for IPC subgroup, instead of A01B 1/2)---results may be buggy otherwise.
#' @examples
...
#  if (destination == "IPC" & (dest.digit > 4){
#    dest.digit <- 6
#    warning(paste("IPC codes only accept 1 digit for section, 3 digits for class, 4 digits for subclass, 6 to 8 digits for group, and 9 to 12 digits for subgroup. If you are using more than 4 digits, it is now reset to subgroup."))
#  }
  ...
  if ((origin == "USPC2012" | origin == "USPC") & (destination == "IPC2012" | destination == "IPC")){
    ...
    if (dest.digit == 2 | dest.digit == 5 | dest.digit > 12) {stop("'dest.digit' only accepts 1-digit inputs for IPC codes (section), 3-digits inputs for IPC codes (class), 4-digits inputs for IPC codes (subclass), 6 to 8-digits inputs for IPC codes (group), and 9 to 12-digits inputs for IPC codes (subgroup), including spaces and regular expressions.")}
    
  } else if ((origin == "IPC2012" | origin == "IPC") & (destination == "USPC2012" | destination == "USPC")) {
    
    if (digits == 2 | digits == 5 | digits > 12) {stop("'sourcevar' only accepts 1-digit inputs for IPC codes (section), 3-digits inputs for IPC codes (class), 4-digits inputs for IPC codes (subclass), 6 to 8-digits inputs for IPC codes (group), and 9 to 12-digits inputs for IPC codes (subgroup), including spaces and regular expressions.")}
