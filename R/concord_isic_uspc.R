#' Converting USPC and ISIC Codes
#'
#' Concords the Unites State Patent Classification codes (USPC 2012) to and from International Standard Industrial Classification codes (ISIC Revision 2, 3, 3.1, 4) via the bridge of NAICS codes.
#'
#' @param sourcevar An input character vector of USPC or ISIC codes. The function accepts 3-digit codes for USPC (class), 5 to 11-digit codes for USPC (subclass), and 1 to 4-digit codes for ISIC.
#' @param origin A string setting the input industry/patent classification: "USPC" (combined), "USPC2012", "ISIC2" (1968), "ISIC3" (1989), "ISIC3.1" (2002), "ISIC4" (2008).
#' @param destination A string setting the output industry/patent classification: "USPC" (combined), "USPC2012", "ISIC2" (1968), "ISIC3" (1989), "ISIC3.1" (2002), "ISIC4" (2008).
#' @param dest.digit An integer indicating the preferred number of digits for output codes. Allows 3-digit codes for USPC (class), 5 to 11-digit codes for USPC (subclass), and 1 to 4-digit codes for ISIC. The default is 3 digits.
...
#' # one-to-one match
#' 
#' concord_isic_uspc("002/2.11", origin = "USPC", destination = "ISIC3", dest.digit = 3)
#' 
#' concord_isic_uspc("002/2.11", origin = "USPC", destination = "ISIC3", dest.digit = 4)
#' 
#' concord_isic_uspc("002/2.11", origin = "USPC", destination = "ISIC4", dest.digit = 3)
#' 
#' concord_isic_uspc("002/2.11", origin = "USPC2012", destination = "ISIC3", dest.digit = 3)
#' 
#' concord_isic_uspc("1711", origin = "ISIC3", destination = "USPC", dest.digit = 3)
#' 
#' concord_isic_uspc("1711", origin = "ISIC3", destination = "USPC", dest.digit = 9)
#' 
#' concord_isic_uspc("1711", origin = "ISIC3", destination = "USPC2012", dest.digit = 3)
#' 
#' concord_isic_uspc("1711", origin = "ISIC3.1", destination = "USPC", dest.digit = 3)
#' 
#' # multiple-to-one match
#' 
#' concord_isic_uspc(c("002/2.11", "072/47"), origin = "USPC", destination = "ISIC3", dest.digit = 3)
#' 
#' concord_isic_uspc(c("002/2.11", "072/47"), origin = "USPC", destination = "ISIC3", dest.digit = 4)
#' 
#' concord_isic_uspc(c("1711", "1721"), origin = "ISIC3.1", destination = "USPC", dest.digit = 3)
#' 
#' concord_isic_uspc(c("1711", "1721"), origin = "ISIC3.1", destination = "USPC", dest.digit = 9)
#' 
#' # one to multiple matches (all = T)
#' 
#' concord_isic_uspc("002/2.11", origin = "USPC", destination = "ISIC4", dest.digit = 4, all = TRUE)
#' 
#' concord_isic_uspc("2826", origin = "ISIC4", destination = "USPC", dest.digit = 3, all = TRUE)
concord_isic_uspc <- function (sourcevar,
                              origin,
                              destination,
                              dest.digit = 3,
                              all = FALSE) {
  ...
  if ((origin == "ISIC2" | origin == "ISIC3" | origin == "ISIC3.1" | origin == "ISIC4") & (destination == "USPC" | destination == "USPC2012")){
    
    dictionary <- concordance::uspc2012_naics2002
    naics.bridge <- "NAICS2002"
    
  } else if ((origin == "USPC" | origin == "USPC2012") & (destination == "ISIC2" | destination == "ISIC3" | destination == "ISIC3.1" | destination == "ISIC4")) {
  ...
  if (length(digits) >= 1){
  if ((origin == "ISIC2" | origin == "ISIC3" | origin == "ISIC3.1" | origin == "ISIC4")){
  ...
  if ((destination == "USPC") | (destination == "USPC2012")){
    ...
    if (all(dest.digit > 4) & all(dest.digit <= 11)) {
