#' Converting IPC and SITC Codes
#'
#' Concords International Patent Classification codes (IPC 2012) to and from Standard International Trade Classification codes (SITC1, SITC2, SITC3, SITC4) via the bridge of USPC and NAICS codes.
#'
#' @param sourcevar An input character vector of IPC or SITC codes. The function accepts 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 9 to 13-digit codes for IPC (group and subgroup), and 1 to 5-digit codes for SITC.
#' @param origin A string setting the input industry/patent classification: "IPC" (combined), "IPC2012", "SITC1" (1950), "SITC2" (1974), "SITC3" (1985), "SITC4" (2006).
#' @param destination A string setting the output industry/patent classification: "IPC" (combined), "IPC2012", "SITC1" (1950), "SITC2" (1974), "SITC3" (1985), "SITC4" (2006).
#' @param dest.digit An integer indicating the preferred number of digits for output codes. Allows 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 9 to 13-digit codes for IPC (group and subgroup), and 1 to 5-digit codes for SITC. The default is 4 digits.
...
#' @examples
#' # one-to-one match
#' concord_sitc_ipc("A41D 19/00", origin = "IPC", destination = "SITC3", dest.digit = 3)
#' 
#' concord_sitc_ipc("A41D 19/00", origin = "IPC", destination = "SITC3", dest.digit = 5)
#' 
#' concord_sitc_ipc("A41D 19/00", origin = "IPC", destination = "SITC4", dest.digit = 3)
#' 
#' concord_sitc_ipc("A41D 19/00", origin = "IPC2012", destination = "SITC3", dest.digit = 3)
#' 
#' concord_sitc_ipc("65112", origin = "SITC3", destination = "IPC", dest.digit = 4)
#' 
#' concord_sitc_ipc("65112", origin = "SITC3", destination = "IPC", dest.digit = 9)
#' 
#' concord_sitc_ipc("65112", origin = "SITC3", destination = "IPC2012", dest.digit = 4)
#' 
#' concord_sitc_ipc("65112", origin = "SITC4", destination = "IPC", dest.digit = 4)
#' 
#' # multiple-to-one match
#' 
#' concord_sitc_ipc(c("A41D", "A41F"), origin = "IPC", destination = "SITC3", dest.digit = 3)
#' 
#' concord_sitc_ipc(c("A41D", "A41F"), origin = "IPC", destination = "SITC3", dest.digit = 4)
#' 
#' concord_sitc_ipc(c("65112", "65113"), origin = "SITC3", destination = "IPC", dest.digit = 4)
#' 
#' concord_sitc_ipc(c("65112", "65113"), origin = "SITC3", destination = "IPC", dest.digit = 9)
#' 
#' # one to multiple matches (all = T)
#' 
#' concord_sitc_ipc("A41D 19/00", origin = "IPC", destination = "SITC3", dest.digit = 4, all = TRUE)
#' 
#' concord_sitc_ipc("65112", origin = "SITC4", destination = "IPC", dest.digit = 4, all = TRUE)
concord_sitc_ipc <- function (sourcevar,
                              origin,
                              destination,
                              dest.digit = 4,
                              all = FALSE) {
  ...
  if ((origin == "SITC1" | origin == "SITC2" | origin == "SITC3" | origin == "SITC4") & (destination == "IPC" | destination == "IPC2012")){
    
    dictionary <- concordance::uspc2012_ipc2012
    uspc.bridge <- "USPC"
    
  } else if ((origin == "IPC" | origin == "IPC2012") & (destination == "SITC1" | destination == "SITC2" | destination == "SITC3" | destination == "SITC4")) {
