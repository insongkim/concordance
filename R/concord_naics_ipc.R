#' Converting IPC and NAICS Codes
#'
#' Concords International Patent Classification codes (IPC 2012) to and from North American Industry Classification System codes (NAICS2002, NAICS2007, NAICS2012, NAICS2017, NAICS combined) via the bridge of USPC codes.
#'
#' @param sourcevar An input character vector of IPC or NAICS codes. The function accepts 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 9 to 13-digit codes for IPC (group and subgroup), and 2 to 6-digit codes for NAICS.
#' @param origin A string setting the input industry/patent classification: "IPC" (combined), "IPC2012", "NAICS2002", "NAICS2007", "NAICS2012", "NAICS2017", "NAICS" (combined).
#' @param destination A string setting the output industry/patent classification: "IPC" (combined), "IPC2012", "NAICS2002", "NAICS2007", "NAICS2012", "NAICS2017", "NAICS" (combined).
#' @param dest.digit An integer indicating the preferred number of digits for output codes. Allows 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 9 to 13-digit codes for IPC (group and subgroup), and 2 to 6-digit codes for NAICS codes. The default is 4 digits.
...
#' # one-to-one match
#' 
#' concord_naics_ipc("A41D 19/00", origin = "IPC", destination = "NAICS", dest.digit = 3)
#' 
#' concord_naics_ipc("A41D 19/00", origin = "IPC", destination = "NAICS", dest.digit = 4)
#' 
#' concord_naics_ipc("A41D 19/00", origin = "IPC", destination = "NAICS2002", dest.digit = 3)
#' 
#' concord_naics_ipc("A41D 19/00", origin = "IPC2012", destination = "NAICS", dest.digit = 3)
#' 
#' concord_naics_ipc("3133", origin = "NAICS", destination = "IPC", dest.digit = 4)
#' 
#' concord_naics_ipc("3133", origin = "NAICS", destination = "IPC", dest.digit = 9)
#' 
#' concord_naics_ipc("3133", origin = "NAICS", destination = "IPC2012", dest.digit = 4)
#' 
#' concord_naics_ipc("3133", origin = "NAICS2007", destination = "IPC", dest.digit = 4)
#' 
#' # multiple-to-one match
#' 
#' concord_naics_ipc(c("A41D", "A41F"), origin = "IPC", destination = "NAICS", dest.digit = 3)
#' 
#' concord_naics_ipc(c("A41D", "A41F"), origin = "IPC", destination = "NAICS", dest.digit = 4)
#' 
#' concord_naics_ipc(c("313", "333"), origin = "NAICS", destination = "IPC", dest.digit = 4)
#' 
#' concord_naics_ipc(c("313", "333"), origin = "NAICS", destination = "IPC", dest.digit = 9)
#' 
#' # one to multiple matches (all = T)
#' 
#' concord_naics_ipc("A41D 19/00", origin = "IPC", destination = "NAICS", dest.digit = 3, all = TRUE)
#' 
#' concord_naics_ipc("3131", origin = "NAICS", destination = "IPC", dest.digit = 4, all = TRUE)
concord_naics_ipc <- function (sourcevar,
                               origin,
                               destination,
                               dest.digit = 4,
                               all = FALSE) {
  ...
  if ((origin == "NAICS" | origin == "NAICS2002" | origin == "NAICS2007" | origin == "NAICS2012" | origin == "NAICS2017") & (destination == "IPC" | destination == "IPC2012")){
    
    dictionary <- concordance::uspc2012_ipc2012
    uspc.bridge <- "USPC"
    
  } else if ((origin == "IPC" | origin == "IPC2012") & (destination == "NAICS" | destination == "NAICS2002" | destination == "NAICS2007" | destination == "NAICS2012" | destination == "NAICS2017")) {
