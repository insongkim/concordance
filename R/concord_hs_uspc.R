#' Converting USPC and HS Codes
#'
#' Concords the Unites State Patent Classification codes (USPC 2012) to and from Harmonized System codes (HS0, HS1, HS2, HS3, HS4, HS5, HS combined) via the bridge of NAICS codes.
#'
#' @param sourcevar An input character vector of USPC or HS codes. The function accepts 3-digit codes for USPC (class), 5 to 11-digit codes for USPC (subclass), and 2, 4, 6-digit codes for HS.
#' @param origin A string setting the input industry/patent classification: "USPC" (combined), "USPC2012", "HS" (combined), "HS0" (1988/92), "HS1" (1996), "HS2" (2002), "HS3" (2007), "HS4" (2012), "HS5" (2017).
#' @param destination A string setting the output industry/patent classification: "USPC" (combined), "USPC2012", "HS" (combined), "HS0" (1988/92), "HS1" (1996), "HS2" (2002), "HS3" (2007), "HS4" (2012), "HS5" (2017).
#' @param dest.digit An integer indicating the preferred number of digits for output codes. Allows 3-digit codes for USPC (class), 5 to 11-digit codes for USPC (subclass), and 2, 4, 6-digit codes for HS. The default is 3 digits.
...
#' # one-to-one match
#' 
#' concord_hs_uspc("002/2.11", origin = "USPC", destination = "HS", dest.digit = 4)
#' 
#' concord_hs_uspc("002/2.11", origin = "USPC", destination = "HS", dest.digit = 6)
#' 
#' concord_hs_uspc("002/2.11", origin = "USPC", destination = "HS3", dest.digit = 4)
#' 
#' concord_hs_uspc("002/2.11", origin = "USPC2012", destination = "HS", dest.digit = 4)
#' 
#' concord_hs_uspc("5205", origin = "HS", destination = "USPC", dest.digit = 3)
#' 
#' concord_hs_uspc("5205", origin = "HS", destination = "USPC", dest.digit = 9)
#' 
#' concord_hs_uspc("5205", origin = "HS", destination = "USPC2012", dest.digit = 3)
#' 
#' concord_hs_uspc("5205", origin = "HS3", destination = "USPC", dest.digit = 3)
#' 
#' # multiple-to-one match
#' 
#' concord_hs_uspc(c("002/2.11", "072/47"), origin = "USPC", destination = "HS", dest.digit = 4)
#' 
#' concord_hs_uspc(c("002/2.11", "072/47"), origin = "USPC", destination = "HS", dest.digit = 6)
#' 
#' concord_hs_uspc(c("500400", "590190"), origin = "HS", destination = "USPC", dest.digit = 4)
#' 
#' concord_hs_uspc(c("500400", "590190"), origin = "HS", destination = "USPC", dest.digit = 9)
#' 
#' # one to multiple matches (all = T)
#' 
#' concord_hs_uspc("002/2.11", origin = "USPC", destination = "HS", dest.digit = 4, all = TRUE)
#' 
#' concord_hs_uspc("5901", origin = "HS", destination = "USPC", dest.digit = 3, all = TRUE)
concord_hs_uspc <- function (sourcevar,
                               origin,
                               destination,
                               dest.digit = 3,
                               all = FALSE) {
  ...
  if ((origin == "HS" | origin == "HS0" | origin == "HS1" | origin == "HS2" | origin == "HS3" | origin == "HS4" | origin == "HS5") & (destination == "USPC" | destination == "USPC2012")){
    
    dictionary <- concordance::uspc2012_naics2002
    naics.bridge <- "NAICS2002"
    
  } else if ((origin == "USPC" | origin == "USPC2012") & (destination == "HS" | destination == "HS0" | destination == "HS1" | destination == "HS2" | destination == "HS3" | destination == "HS4" | destination == "HS5")) {
  ...
  if ((destination == "USPC") | (destination == "USPC2012")){
    ...
    if (all(dest.digit > 4 & dest.digit <= 11)) {
