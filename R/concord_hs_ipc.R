#' Converting IPC and HS Codes
#'
#' Concords International Patent Classification codes (IPC 2012) to and from Harmonized System codes (HS0, HS1, HS2, HS3, HS4, HS5, HS combined) via the bridge of USPC and NAICS codes.
#'
#' @param sourcevar An input character vector of IPC or HS codes. The function accepts 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 9 to 13-digit codes for IPC (group and subgroup), and 2, 4, 6-digit codes for HS.
#' @param origin A string setting the input industry/patent classification: "IPC" (combined), "IPC2012", "HS" (combined), "HS0" (1988/92), "HS1" (1996), "HS2" (2002), "HS3" (2007), "HS4" (2012), "HS5" (2017).
#' @param destination A string setting the output industry/patent classification: "IPC" (combined), "IPC2012", "HS" (combined), "HS0" (1988/92), "HS1" (1996), "HS2" (2002), "HS3" (2007), "HS4" (2012), "HS5" (2017).
#' @param dest.digit An integer indicating the preferred number of digits for output codes. Allows 1-digit codes for IPC (section), 3-digit codes for IPC (class), 4-digit codes for IPC (subclass), 9 to 13-digit codes for IPC (group and subgroup), and 2, 4, 6-digit codes for HS. The default is 4 digits.
...
#' # one-to-one match
#' 
#' concord_hs_ipc("A41D 19/00", origin = "IPC", destination = "HS", dest.digit = 4)
#' 
#' concord_hs_ipc("A41D 19/00", origin = "IPC", destination = "HS", dest.digit = 6)
#' 
#' concord_hs_ipc("A41D 19/00", origin = "IPC", destination = "HS3", dest.digit = 4)
#' 
#' concord_hs_ipc("A41D 19/00", origin = "IPC2012", destination = "HS", dest.digit = 4)
#' 
#' concord_hs_ipc("5205", origin = "HS", destination = "IPC", dest.digit = 4)
#' 
#' concord_hs_ipc("5205", origin = "HS", destination = "IPC", dest.digit = 9)
#' 
#' concord_hs_ipc("5205", origin = "HS", destination = "IPC2012", dest.digit = 4)
#' 
#' concord_hs_ipc("5205", origin = "HS3", destination = "IPC", dest.digit = 4)
#' 
#' # multiple-to-one match
#' 
#' concord_hs_ipc(c("A41D 19/00", "A41F 9/00"), origin = "IPC", destination = "HS", dest.digit = 4)
#' 
#' concord_hs_ipc(c("A41D 19/00", "A41F 9/00"), origin = "IPC", destination = "HS", dest.digit = 6)
#' 
#' concord_hs_ipc(c("500400", "590190"), origin = "HS", destination = "IPC", dest.digit = 4)
#' 
#' concord_hs_ipc(c("500400", "590190"), origin = "HS", destination = "IPC", dest.digit = 9)
#' 
#' # one to multiple matches (all = T)
#' 
#' concord_hs_ipc("A41D 19/00", origin = "IPC", destination = "HS", dest.digit = 4, all = TRUE)
#' 
#' concord_hs_ipc("5901", origin = "HS", destination = "IPC", dest.digit = 4, all = TRUE)
concord_hs_ipc <- function (sourcevar,
                              origin,
                              destination,
                              dest.digit = 4,
                              all = FALSE) {
  ...
  if ((destination == "IPC" | destination == "IPC2012") & (dest.digit == 2 | (dest.digit == 5) | dest.digit >= 14)){
    dest.digit <- 9
    warning(paste("IPC codes only accept 1 digit for section, 3 digits for class, 4 digits for subclass, 6 to 8 digits for group, and 9 to 13 digits for subgroup. If you are using more than digits other than these, it is now reset to subgroup."))
  }
  
  # load corresponding conversion dictionary and HS bridge based on years
  if ((origin == "HS" | origin == "HS0" | origin == "HS1" | origin == "HS2" | origin == "HS3" | origin == "HS4" | origin == "HS5") & (destination == "IPC" | destination == "IPC2012")){
    
    dictionary <- concordance::uspc2012_ipc2012
    uspc.bridge <- "USPC"
    
  } else if ((origin == "IPC" | origin == "IPC2012") & (destination == "HS" | destination == "HS0" | destination == "HS1" | destination == "HS2" | destination == "HS3" | destination == "HS4" | destination == "HS5")) {
