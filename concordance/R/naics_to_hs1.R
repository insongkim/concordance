#' Converts NAICS to HS1 Codes
#'
#' \code{concord_naics_hs1} converts NAICS (combined) to HS1 codes.
#'
...
#' @param destination A string setting the output industry classification (HS1).
...
#' concord_naics_hs1(sourcevar = "111120", origin = "NAICS", destination = "HS1", all = FALSE)
#' concord_naics_hs1(sourcevar = "111120", origin = "NAICS", destination = "HS1", all = TRUE)
#'
#' # two inputs: one-to-multiple match
#' concord_naics_hs1(sourcevar = c("111120", "326199"), origin = "NAICS", destination = "HS1", all = FALSE)
#' concord_naics_hs1(sourcevar = c("111120", "326199"), origin = "NAICS", destination = "HS1", all = TRUE)
#'
#' # two inputs: repeated
#' concord_naics_hs1(sourcevar = c("111120", "111120"), origin = "NAICS", destination = "HS1", all = FALSE)
#' concord_naics_hs1(sourcevar = c("111120", "111120"), origin = "NAICS", destination = "HS1", all = TRUE)
#'
#' # sourcevar has different number of digits, will give an error
#' concord_naics_hs1(sourcevar = c("111120", "11112"), origin = "NAICS", destination = "HS1", all = FALSE)
#'
#' # If no match, will return NA
#' concord_naics_hs1(sourcevar = c("111120", "111121"), origin = "NAICS", destination = "HS1", all = FALSE)
#'
#' # 4-digit inputs
#' concord_naics_hs1(sourcevar = c("1111", "3271"), origin = "NAICS", destination = "HS1", all = FALSE)
#'
#' # 4-digit outputs
#' concord_naics_hs1(sourcevar = c("111120", "326199"), origin = "NAICS", destination = "HS1", dest.digit = 4, all = FALSE)
concord_naics_hs1 <- function (sourcevar,
                               origin,
                               destination,
                               dest.digit = 6,
                               all = FALSE) {
  ...
  if ((!dest.digit %in% destination.digits)) {stop("'dest.digit' only accepts 2, 4, 6-digit outputs for HS1 codes.")}
  ...
  if (all == TRUE){
    ...
    out.merge <- nest_join(tibble(NAICS = sourcevar),
                           dest.var,
                           by = "NAICS")
    ...
    out <- dest.var.sub[match(sourcevar, dest.var.sub$NAICS), "match"] %>%
      pull(match)
  }
