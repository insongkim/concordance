...
getRauch <- function(sourcevar, origin="hs", setting="CON", verbose=FALSE) {
    if(origin == "naics"){
        return(getRauch_naics(naics=sourcevar, setting=setting, verbose=verbose))
    } else if (origin == "hs") {
        return(getRauch_hs(hs=sourcevar, setting=setting, verbose=verbose))
    }
}


getRauch_hs <- function(hs, setting="CON", verbose=FALSE) {
    ## sitc <- concord(hs, "hs", "sitc2")
    sitc <- hs2sitc[which(hs2sitc$hs %in% hs),"sitc2"]
...
getRauch_naics <- function(naics, setting="CON", verbose=FALSE){
    ...
    if(length(hs6s) > 0){
        ...
        for(i in 1:length(uniq.hs6s)){
            ## print(i)
            rauch.hs[i] <- getRauch_hs(uniq.hs6s[i], setting=setting)
        }
